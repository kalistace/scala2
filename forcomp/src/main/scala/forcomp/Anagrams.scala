...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase.trim.foldLeft(Map.empty[Char, Int] withDefaultValue (0)) { (occurences, x) =>
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case (char, times) :: xs =>
      for {
        ...
        charSubset <- times to 0 by -1
      } yield (if (charSubset == 0) rest else (char, charSubset) :: rest)
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val precondition = y.exists(yElem => x.exists(xElem => yElem._1 == xElem._1))
    if (precondition) {
      val yMap = y.toMap withDefaultValue 0
      for {
        (char, int) <- x
        difference = int - yMap(char)
        if difference > 0
      } yield {
        (char, difference)
      }
